...
//
 export const fetchText = createAsyncThunk('pages/fetchText', async (payload) => {
    ...
    try {
        const res = await axios.get(`https://childrens-interactive-reader.onrender.com/books/text/${payload.bookId}/${payload.page}`);
  ...
  export const fetchBookData = createAsyncThunk('pages/fetchBookInfo', async (payload) => {
    try {
        ...
        const res = await axios.get(`https://childrens-interactive-reader.onrender.com/books/${book_id}`);
       return res.data;
    } catch (err) {
        console.log(err);
    }
...
export  const fetchNumberOfPages = createAsyncThunk('pages/fetchNumberOfPages', async (payload) => {
    try {
        const res = await axios.get(`https://childrens-interactive-reader.onrender.com/books/pages/${payload}`);
